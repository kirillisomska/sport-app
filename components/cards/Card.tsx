import { Company, Event, EventType } from "@prisma/client";
...
const Card = ({ event, company, type }: PropTypes) => {
  return (
    <div className=" bg-white border border-gray-200 rounded-lg shadow">
      <a href="#">
        <Image
          className="rounded-t-lg max-h-[200px] object-fill"
          src={event.logo}
          alt=""
          width={700}
          height={500}
      ...
      <div className="p-5">
        ...
        <p className="text-md text-gray-700 flex items-center my-2 gap-2">
          <svg
          ...
          {event.location?.length
            ? event.location
            : "Место проведения не задано"}
        </p>
        <div className="w-full flex justify-between">
          {company ? (
            <p className="mb-3 font-normal text-gray-700 flex items-center gap-2">
              <svg
                fill="#3b82f6"
                width="16px"
                height="16px"
          ...
          {type ? (
            <p className="mb-3 font-normal text-gray-700 ">{type.value}</p>
          ) : null}
        </div>
        <Link
